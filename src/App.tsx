...
import useInfiniteScroll from "./hook/useInfiniteScroll.tsx";
  ...
  async function getItemsOnScroll() {
    try {
      ...
      setItems(
        _sumList.filter(
          (arr, index, callback) =>
            index === callback.findIndex((t) => t.goodsNo === arr.goodsNo)
        )
      );
      page += 1;
    } catch (err) {
    ...
    <div className="App">
      <header className="App-header">
        ...
        <div className="btn__group">
          ...
          <button
            className={"btn__style" + (showSale ? " clicked" : "")}
            onClick={() => {
          ...
          <button
            className={"btn__style" + (showExclusive ? " clicked" : "")}
            onClick={() => {
          ...
          <button
            className={"btn__style" + (showSoldOut ? " clicked" : "")}
            onClick={() => {
