...
export default function ItemList({ items, showSoldOut, showExclusive, showSale, init }) {
  return (
    <>
    {items.length ? items.map((item, i) => showItem(item, i, showSoldOut, showExclusive, showSale, init))
    : <div className="no-item"><img src={require("../assets/not-found.png")}/><div>검색 결과 없음</div></div>
    }
    </>
  );
}
...
function showItem(item, i, showSoldOut, showExclusive, showSale, init) {
   if (!showSoldOut && item.isSoldOut) return null // 품절상품
   if (!init) {
     // 버튼이 1개이상 선택된 상태
     if ((!item.isSale && !item.isExclusive && !item.isSoldOut) && (!(!showSale && !showExclusive))) return; // 세일/단독/품절이 아닌 상품
